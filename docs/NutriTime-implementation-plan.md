...
> 2026-08-24 执行边界：本阶段按已确认需求只完成 Android 手机端完整周期，不修改 `apps/wear`。默认 16:8 可改为 14:10 等整小时比例，活动开始时间也可修改。现有 Wear v1 继续只支持 idle/fasting；手机进入 eating 时兼容提交 idle，Wear eating 与协议升级留待后续。
